...
public class StringUtils {
    ...
    public static <K, V> String getTable(Map<K, V> map, String keyHeader, String valueHeader) {
        ...
        int columnValueSize = Math.max(getStringMaxSize(map.values()), valueHeader
                .length());
        ...
        for (Map.Entry<K, V> entry : map.entrySet()) {
            table += getLine(entry.getKey(), entry.getValue(), columnKeySize, columnValueSize, SPACE);
            table += getRowSeparator(columnKeySize, columnValueSize);
        }
        return table;
    }

    private static <K, V> String getLine(K key, V value, int columnKeySize, int columnValueSize, String character) {
...
//        String rowSeparator = VERTICAL_BORDER;
//        for (int columnWidth : columnWidths) {
//            rowSeparator += addString(HORIZONTAL_BORDER, columnWidth);
//            rowSeparator += VERTICAL_BORDER;
//        }
//        rowSeparator += LINE_SEPARATOR;
//        return rowSeparator;
